...
const COLORS = ["#8B9E77", "#C8B8A2", "#6B5344", "#A89880", "#B5C4A1", "#D4C8B8"];
...
export default function PieChartTW({ holdings }: Props) {
  const data = holdings.map((h) => ({ name: `${h.ticker} ${h.stockName}`, value: h.valueTwd }));

  return (
    <Card>
      <p className="text-sm font-semibold text-[#6B5344] mb-4">台股持股分布</p>
      {data.length === 0 ? (
